...
{
    public class SqliteQueryFactory:IQueryFactory
    {
        public string CreateTable(Type type, string? table = "")
        {
        ...
        public Tuple<string, DynamicSqlParameter> Delete<T>(T obj)
        {
        ...
        public Tuple<string, List<DynamicSqlParameter>> BulkInsert<T>(List<T> objList, string? table = null)
        {
            try
            {
                var tableNameT = ReflectionFactory.GetTableAttribute(typeof(T));
        ...
        public Tuple<string, string, string> QueryWildCardHelper(string colName, string paramName, string paramValue)
        {
            var op = "=";
